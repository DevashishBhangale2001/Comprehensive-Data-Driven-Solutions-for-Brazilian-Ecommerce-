...
def load_model():
    with open(r'C:\Users\Lenovo\Desktop\CDAC DBDA\final\myenv\models\recommendation_model.pkl', 'rb') as model_file:
        algo = pickle.load(model_file)
    with open(r'C:\Users\Lenovo\Desktop\CDAC DBDA\final\myenv\models\id_mapping.pkl', 'rb') as id_mapping_file:
...
def get_recommendations(algo, df, selected_customer_id):
    ...
    # Create a test set for prediction
...
def main():
    ...
    # Filter out NaN values in the customer_id column
    df = df.dropna(subset=['customer_id'])
    # Debugging: Check if there are still any NaN values
    if df['customer_id'].isna().any():
        st.error("There are still NaN values in customer_id after dropna.")
    ...
    df['customer_id'] = df['customer_id'].map(id_mapping)
     # Ensure there are no NaN values in customer_id after mapping
    if df['customer_id'].isna().any():
        st.error("NaN values found in customer_id after mapping.")
